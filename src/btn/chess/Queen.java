...
public class Queen extends Piece {
	...
	/*
	 * Checks whether a position is reachable for the Queen at its current position.
	 * @param board: Board that the Queen currently resides on
	 * @param newPos: Vector2i specifying the position to check to see if the Queen can reach
	...
	public boolean isMoveValid(Board board, Vector2i newPos) {
		...
		if ((xDisp == 0 && yDisp == 0) ||
			(xDisp != 0 && yDisp != 0 && Math.abs(xDisp) != Math.abs(yDisp))) {
			return false;
		}
